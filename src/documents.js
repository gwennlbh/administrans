...
export const fields = Object.entries(fieldsById).map(([id, config]) => ({ id, ...config }))

/**
 * @typedef {keyof typeof fieldsById} FieldID
 */

/**
 * @typedef {'text'|'textarea'|'date'|'checkbox'|'select'|'email'} InputType
 */

/**
 * @template {InputType} T
 * @typedef {T extends 'text' ? string : T extends 'textarea' ? string : T extends 'date' ? string : T extends 'checkbox' ? boolean : T extends 'select' ? string : T extends 'email' ? string : never} DatatypeFromInputType
 */

/**
 * @template {FieldID} ID
 * @typedef {typeof fieldsById[ID]['type'] extends 'select' ? typeof fieldsById[ID]['choices'][number]['value'] : DatatypeFromInputType<typeof fieldsById[ID]['type']>} DatatypeOfField
 */
...
/**
 * @typedef {{[ID in FieldID]: DatatypeOfField<ID>}} FormData
 */
 ...
 * @property {Array<{label: string, value: string}>} [choices]
 * @property {(data: FormData) => void} [action]
 * @property {{ rows: number }} [attributes]
 * @property {string} [placeholder]
 * @property {boolean} [persist=true]
 * @property {string} [help]
 * @property {(data: FormData) => boolean} [showCondition]
 */

/**
 *
 * @param {FieldID | ({} & string)} id identifiant du champ
 * @param {FieldConfig} [params={}]
 * @returns
 */
  ...
  },
  {
    ...
    structure: [
      ...
      field('enfants', {
        ...
        showCondition: (data) => {
          return data.majActeMariageEnfants || data.majActeNaissanceEnfants
        },
        type: 'textarea',
